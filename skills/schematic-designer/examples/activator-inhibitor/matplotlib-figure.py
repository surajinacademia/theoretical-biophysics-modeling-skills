...
The script follows the schematic-designer ``learning-dynamics-2406`` profile
...
def panel_heading(ax, letter, title):
    """Add the profile's bold lower-case panel tag and short heading."""
    ax.text(
        -0.13,
        1.045,
        f"({letter})",
...
def draw_schematic(ax):
    ...
    ax.add_patch(core)
    ax.text(0.0, 0.08, r"$u$", ha="center", va="center", fontsize=11.0, zorder=8)
    ...
    for side in (-1.0, 1.0):
        ...
        ax.add_patch(v_node)
        ax.text(x_v, 0.08, r"$v$", ha="center", va="center", fontsize=9.5, zorder=8)
        x_bar = 0.43 * side
        ax.plot(
            [x_v - 0.25 * side, x_bar],
            [0.08, 0.08],
...
def draw_profiles(ax, x, u, v):
    ...
    ax.plot(x, u, color=STYLE_BLUE_DARK, lw=1.55, solid_capstyle="round", zorder=4)
    ax.plot(
        x,
        v,
        color=STYLE_RED_DARK,
    ...
    ax.axhline(0.0, color=STYLE_INK, lw=0.7, zorder=1)
...
def draw_net_field(ax, cax, x, y, net):
    ...
    colorbar.set_label(r"net field $u-v$", fontsize=8.0, labelpad=1.0)
