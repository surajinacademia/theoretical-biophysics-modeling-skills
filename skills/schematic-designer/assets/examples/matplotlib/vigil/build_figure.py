...
def write_source(path, text):
    """Atomically replace generated provenance; never follow an existing link."""
    if path.is_symlink() or (path.exists() and not path.is_file()):
        raise RuntimeError(f'Unsafe generated-source target: {path}')
...
from vector_output import prepare_output_directory, save_figure_atomic
OUT = prepare_output_directory(args.output_dir)
SOURCE = prepare_output_directory(OUT/'source')
...
write_source(SOURCE/'vigil-minimalist.tex', '\n'.join(lines)+'\n')
write_source(SOURCE/'minimalist-profile.sty', style.tikz_style())
write_source(SOURCE/'geometry-check.json', json.dumps({'seed':42,'cloud_points_each':len(g['P_m']),'trajectory_points':len(g['x_ours']),'stars_each':len(g['S_m']),'vig_gap':float(g['see_star']-g['blind_star']),'bounds':[float(g[k]) for k in ('xmin','xmax','ymin','ymax')],'width_inches':W,'height_inches':H},indent=2)+'\n')
plt.close(fig)
print(OUT)
