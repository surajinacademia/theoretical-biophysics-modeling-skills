...
% Native TeX/TikZ specimen: learning-dynamics layout with Minimalist roles.
...
\tikzset{paneltag/.append style={anchor=west}}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % Headings and whitespace separate the scientific phases. ------------------
  \node[majorheading] at (8.9,7.12) {Adaptive field response};

  \node[sectiontitle,text=Train] at (5.675,6.31) {Training};
  \node[sectiontitle,text=Retrieve] at (14.76,6.31) {Retrieval};

  % Panel (a): imposed motion -------------------------------------------------
  \node[paneltag] at (0.05,5.35) {A};
  ...
  \end{scope}
  \draw[snapshot] (12.45,5.05) -- (12.45,1.02) -- (17.48,1.02);

  \draw[Retrieve,line width=\msLineContext,opacity=\msOpacityContext]
  ...
  \foreach \p/\op in {{(13.12,1.62)}/0.28,{(13.65,2.00)}/0.38,
                         ...
                         {(15.20,3.10)}/0.82}{
    \fill[Retrieve,opacity=\op] \p circle (0.5*\msMarkerPoint);
  }

  \node[entityi,draw opacity=\msOpacityContext,fill opacity=\msOpacityFill,text opacity=1] at (13.12,1.62) {$i$};
