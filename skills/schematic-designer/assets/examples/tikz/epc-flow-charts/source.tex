...
\tikzset{state/.style={rectangle,draw=msColor5,text=msInk,fill=msPaper,ms structure,minimum height=2em,inner sep=\msSpaceLabel,align=left}}
\newcommand{\stateheading}[1]{{\sffamily\fontsize{\msFontTitle}{10.8pt}\selectfont\bfseries\textcolor{msColor5}{#1}}}
\setlength{\arrayrulewidth}{\msLineContext}
\begin{document}

\begin{tikzpicture}[ms base,ms arrow]
\begin{scope}
\node[ms panel,anchor=east] at (-3,2.5) {A};
 ...
 {\begin{tabular}{l}
  \stateheading{Query}\\
  \parbox{4cm}{\begin{itemize}\setlength{\itemsep}{\msSpaceUnit}\setlength{\parskip}{0pt}
   \item Start
   \item Parameter $Q$
   \item Zufallszahl aus \mbox{$[0, 2^Q-1]$} in Slotzähler $SC$
  \end{itemize}
  }\\[\msSpacePanel]
  \stateheading{QueryAdjust}\\
  \parbox{4cm}{\begin{itemize}\setlength{\itemsep}{\msSpaceUnit}\setlength{\parskip}{0pt}
   \item Variiere Q
 ...
 \node[state,    	% layout (defined above)
  text width=3.6cm, 	% max text width
 ...
 \begin{tabular}{l} 	% content
  \stateheading{Ack}\\
  \parbox{3.4cm}{Bestätigen mit $RN_{16}$}
 \end{tabular}
 };
 ...
 \node[state,
  ...
  yshift=-2cm,
  anchor=center,
  text width=3.6cm] (QUERYREP)
 {%
 \begin{tabular}{l}
  \stateheading{QueryRep}\\
  \parbox{3.4cm}{Dekrementiere Slotzähler}
 \end{tabular}
 };
 ...
 (QUERYREP)  	edge                node[anchor=west,right]{$SC_n = 0$} (ACK);

\end{scope}
\begin{scope}[yshift=-9cm]
\node[ms panel,anchor=east] at (-3,3.5) {B};
 ...
 \begin{tabular}{l}     % content
  \stateheading{RAKE}\\
  \parbox{3.4cm}{Paralleles Entspreizen aller Sequenzen aus $T_P$}
 \end{tabular}
 };
 ...
 \path (QUERY) edge[bend left=50]  node[anchor=south,above,text width=4cm]
                   {
                   \begin{itemize}\setlength{\itemsep}{\msSpaceUnit}\setlength{\parskip}{0pt}
                    \item Wähle Zeile $i$ aus $T_P$
                    \item sende $RN_{16}$ mit $C_i$
                   \end{itemize}
                   } (RAKE)
 (RAKE) edge                    (ACK)
 (ACK)  edge                    node[anchor=east,right,text width=3.6cm,xshift=1em]
                  {
                  \begin{itemize}\setlength{\itemsep}{\msSpaceUnit}\setlength{\parskip}{0pt}
